    ...
    return new Promise(function(resolve, reject) {
        ...
        if (action.method != undefined) {
            ...
            switch (propertyExists(action.method, methodPossibleItems)) {
                ...
                case 'scheduler':
                    var schedulerPossibleItems = ["setScheduler"]; 
                    switch (propertyExists(action.method.scheduler, schedulerPossibleItems)) {

                        case 'setScheduler':
                            return actionsModule.worker.setScheduler(action.method.scheduler.setScheduler, _WFInstance, uuid).then(function(result) {
                                resolve(result.data);
                            }, function(err) {
                                reject(err);
                            });
