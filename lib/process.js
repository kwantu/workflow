			...
			library.getUsersListByRole(profileId, role).then(function(list){
				if(list != undefined){
					if(list.length > 1){
						...
						makeUserAssignment(spuuid, userId, username).then(function(data){
							console.log(data);
							resolve(data);
						}, function(err){
							console.log(err);
...
function makeUserAssignment(spuuid, userId, username){
	...
	return new Promise(function(resolve, reject) {
		try {
			console.log('spuuid==='+ spuuid);

			var spObject = JSON.xpath("/subprocesses[_id eq '"+ spuuid +"']",app.SCOPE.workflow, {})[0];
			 
			console.log('spuuid doc get');
			console.log(spObject);
			var assignee = JSON.xpath("/step/assignedTo",spObject,{})[0];
			assignee.name = username+"";
			assignee.userId = userId+"";
			
			dao.get(spuuid+":notificationRequest").done(function(data){

				data.source = "remote";
				data.message = NOTIFICATION_PROFILE_MSG_REASSIGNMENT;
				data.senderUserId = LOCAL_SETTINGS.SUBSCRIPTIONS.userId;

				dao.upsert(data).done(function(data){
					console.log(data);
					resolve('Auto Assignment done.');
				}).fail(function(err){
					console.log(err);
					reject(err);
				});
