    ...
    return new Promise(function(resolve, reject) {
        try {
            ...
            if(model.message == undefined){
                model.message = step.setInstanceStatusTo[instanceStatus].label.i18n["en"];
            }
...
function indicators(indicators, _WFInstance, spuuid) {
    ...
    return new Promise(function(resolve, reject) {
        try {
            ...
            if(array.length == 0){
                array = JSON.xpath("/indicators[_id = (/subprocesses[_id  eq '"+spuuid+"']/indicators/instances/uuid)]", _WFInstance, {});
            }
...
function action(action, processId, processSeq, subProcessId, subProcessSeq, subProcess, step, _WFInstance, data, uuid) {
    return new Promise(function(resolve, reject) {
        ...
        if (action.method != undefined) {
            var methodPossibleItems = ["form", "indicator", "profile", "subProcessInstance", "step", "community", "application", "user", "sdo", "performance", "taxonomy", "variables", "notification", "report", "worker", "participants", "generalFunctions", "taskStatus", "scheduler"];
            switch (propertyExists(action.method, methodPossibleItems)) {
                ...
                case 'worker':
                    var workerPossibleItems = ["sendWorker", "executeLocal", "create", "service"];
                    switch (propertyExists(action.method.worker, workerPossibleItems)) {
                        ...
                        case 'service':
                            ...
                            workerObject.userId = _lclx.SUBSCRIPTIONS.userId;
                            workerObject.profileId = app.profile._id;
                            workerObject.subProcessUUID = uuid;
                            workerObject.subprofileId = app.profile.subprofileId;
                            workerObject.seq = subProcessSeq;
                            workerObject.groupKey = spProcessObject.groupKey;
                            
                            var populateParam = function(p){
                                if(action.method.worker.service.params == undefined || p == action.method.worker.service.params.length){
                                    var URL = DOMAIN.replace('/exist/rest', "") + action.method.worker.service.url;
                                    
                                    var options = {
                                        url: URL,
                                        method: "POST",
                                        data: JSON.stringify(workerObject),
                                    ...
                                    .catch(function (err) {
                                        console.log(err);
                                        reject("Error while performing scheduler setup.");
                                        
                                    });
                                }
                                else{
                                    helper.getNodeValue(action.method.worker.service.params[p].value, _WFInstance, uuid)
                                    .then(function(value) {
                                        workerObject[action.method.worker.service.params[p].key] = value; 
                                        populateParam(p + 1);
                                    }).catch(function(err) {
                                        reject(err);
                                    });
    ...
    return new Promise(function(resolve, reject) {
        ...
        preActions(preActionsConf, _WFInstance, spuuid).then(function(preActionResult) {
            ...
            if (task.assign.role != undefined) {
                ...
                var processToAssignExternal = function() {
                    ...
                    library.getUsersFromProfileRole(id, role).then(function(list) {
                        ...
                        if (list != undefined) {
                            ...
                            if(task.assign.role.restrict != undefined){
                                if(task.assign.role.restrict.type == "stepUser"){
                                    var stepUsers = JSON.xpath("/subprocesses[_id eq '" + spuuid + "']/history[id eq '" + task.assign.role.restrict.stepId + "']/transition/userId", _WFInstance, {});
                                    if(task.assign.role.restrict.operator == "=="){
                                        for (var i = 0; i < list.length; i++) {
                                            if (stepUsers.indexOf(list[i].id) == -1) {
                                                list.splice(i, 1);
                                                i--;
                                            }
                                        }
                                    }
                                    else if(task.assign.role.restrict.operator == "!="){
                                        for (var i = 0; i < list.length; i++) {
                                            if (stepUsers.indexOf(list[i].id) != -1) {
                                                list.splice(i, 1);
                                                i--;
                ...
                var userId = assignmentObject == undefined ? "" : assignmentObject.userId;
                var username = assignmentObject == undefined ? "" : assignmentObject.name;
...
function transition(processId, processSeq, subProcessId, subProcessSeq, stepId, transitionId, data, _WFInstance, spuuid, model) {
    return new Promise(function(resolve, reject) {
        try {
            ...
            if(transition[0]?.restrict != undefined){
                if(transition[0].restrict.type == "stepUser"){
                    var stepUsers = JSON.xpath("/subprocesses[_id eq '" + spuuid + "']/history[id eq '" + transition[0].restrict.stepId + "']/transition/userId", _WFInstance, {});
                    if(transition[0].restrict.operator == "=="){
                        if(stepUsers.indexOf(_lclx.SUBSCRIPTIONS.userId) == -1){
                            
                            reject({"error": "You are not authorised to perform this transition."});
                        }
                    }
                    else if(transition[0].restrict.operator == "!="){
                        if(stepUsers.indexOf(_lclx.SUBSCRIPTIONS.userId) != -1){
            ...
            var sendTransitionNotifications = async function(transition){
                if(transition?.notifications == undefined){
                    return {};
                }
                else{
                    for(var n = 0; n < transition?.notifications.length; n++){
                        const notificationConfig = await dao.getConfig(transition?.notifications[n]);
                        await actionsModule.notification.sendNotificationWorker(notificationConfig, _WFInstance, spuuid);
                    }
                }
            };
            ...
            if (transition[0].transitionAction.goToStep != undefined) {
                ...
                step(processId, processSeq, subProcessId, subProcessSeq, nextId, nextSeq, data, _WFInstance, spuuid).then(function(result) {
                    ...
                    sendTransitionNotifications(transition[0]).then(function(n) {
...
/*function assignToUsers(message, channelArray){
     ...
     var notification =  { 
          ...
          "senderUserName": _lclx.SESSION.firstName + " " + _lclx.SESSION.lastName,
       };
