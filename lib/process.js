...
function transition(processId, processSeq, subProcessId, subProcessSeq, stepId, transitionId, data, _WFInstance, spuuid, model) {
    return new Promise(function(resolve, reject) {
        try {
            ...
            if(transition.restrict != undefined){
                if(transition.restrict.type == "stepUser"){
                    var stepUsers = JSON.xpath("/subprocesses[_id eq '" + spuuid + "']/history[id eq '" + transition.restrict.stepId + "']/transition/userId", _WFInstance, {});
                    if(transition.restrict.operator == "=="){
                    ...
                    else if(transition.restrict.operator == "!="){
                        if(stepUsers.indexOf(_lclx.SUBSCRIPTIONS.userId) != -1){
                            var error = util.error('WFTransitionError', 'You are not authorized to perform this transition.');
                            reject(error);
                        }
