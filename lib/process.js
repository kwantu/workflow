...
function preRequisite(prerequisite, _WFInstance, spuuid) {
    return new Promise(function(resolve, reject) {
        ...
        if (prerequisite.check.numberProcessInstances != undefined) {
            ...
            if(_filterValue == "all"){
                innerXpath = "/" + _filterElement;
            } else {
                innerXpath = "/" + _filterElement + "[. eq '" + _filterValue + "']";
            }
            ...
            helper.getNodeValue(prerequisite.check.indicator.data, _WFInstance, uuid).then(function(dataValue) {

                var compare = util.compare(subjectValueCalculated, prerequisite.check.variable.operator, dataValue);
                if (compare) {
                    var success = util.success('Variable Pre-requisites passed.', {});
                    resolve(success);
                } else {
