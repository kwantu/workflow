...
function task(task, spuuid){
	...
	return new Promise(function(resolve, reject) {
		...
		if(task.assign.profileRole != undefined){
			...
			library.getUsersListByRole(profileId, role).then(function(list){
				if(list != undefined){
					if(list.length > 1){
						...
						var process_CurrentStepName = JSON.xpath("/steps[_seq eq '"+ stepSeq +"']/name/i18n[1]/value",configSubProcess,{});
						var processName = configSubProcess.name.i18n[0].value;

						var spFunc = "<a href=&#34;javascript;&#34; style=&#34;color:blue&#34; onClick=&#34;app.openSubprocess('"+profileId+"','"+ processId +"', '"+ processSeq +"', '"+ processName +"', '"+ subprocessID +"', '"+ subprocessSEQ+"', '"+ subprocessUUID +"', '"+ process_ValidDate +"', '"+ process_CurrentStepName +"')&#34;>click here</a>";
						console.log('spFunc');
						console.log(spFunc);
						finalMsg = finalMsg.replace('#SP_INS_LINK', spFunc);
						...
						if(task.assign.assignment != undefined){
							...
							var assignment = JSON.xpath("/step/assignment",spObject,{})[0];
							var accept = {
