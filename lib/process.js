...
function preRequisite(prerequisite, _WFInstance, spuuid) {
    return new Promise(function(resolve, reject) {
        ...
        if (prerequisite.check.numberProcessInstances != undefined) {
            ...
            var fullPath = "count(/instance/processes/subProcesses[id eq '" + _subprocessId + "']" + innerXpath + ")";
            //var fullPath  = "count(/instance/processes/subProcesses[id eq '"+ _subprocessId +"' and complete = 'true'])";
...
function subProcess(uuid, processId, processSeq, subProcessId, subProcessSeq, subprofileId, data, _WFInstance) {
    ...
    if (baseUUID != undefined && baseUUID != '' && baseUUID.length > 0) {
        ...
        if(JSON.xpath("/subprocesses[_id eq '" + baseUUID + "']", app.SCOPE.workflow, {}).length == 0){
            throw Error("SP WORK"); 
        }
...
function preWorkActions(preWorkActions, _WFInstance) {
    return new Promise(function(resolve, reject) {
        ...
        try {
            ...
            if(JSON.xpath("/subprocesses[_id eq '" + app.SCOPE.processUUID + "']", app.SCOPE.workflow, {}).length == 0){
                throw Error("SP WORK"); 
            }
