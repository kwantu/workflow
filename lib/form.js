...
function create(args){
	...
	return new Promise(function(resolve, reject) {
		...
		var formCreateFn = function(id, indicatorType, indicatorId, validDate){
					...
					if(!object.model._id.endsWith(':approved') && !object.model._id.endsWith(':rejected')){
						...
						gatekeeper.persist(docArray).then(function(savedArray){
							...
							dao.get(mainId).done(function(data){
								...
								gatekeeper.instantiateData(mainId, 'fromDefinition',indicatorModel, data.model.pending.seq).then(function(data){
		...
		for(var counter = 0; counter < indicators.length; counter++)
		{
			...
			var id = '';
	   		...
	   		if(createType == 'newIndicator'){

	   			id = indicatorId + '-' +generateUUID();

		   	} else {

		   		if (indicatorType == 'newSequence') {
					    _WFInstance.indicators.filter(function(indicator){
					    if (indicator.category.term == indicatorId) {
					       indicatorDoc = indicator;
					    }
				    })
				    if (indicatorDoc._id !== undefined) {
				    	 id = indicatorDoc._id;
				    } else {
				     //id = _WFInstance.profile + ':' + _WFInstance.app + ':' + indicatorId + ':0';
				     id = indicatorId + '-' +generateUUID();
				    }
			    } else {
			...
			formCreateFn(id, indicatorType, indicatorId, '');
...
function authorise(form){
	...
	//console.log('----------------------- Form authorise from Workflow -----------------------');
	...
	return new Promise(function(resolve, reject) {
		...
		for(var i=0; i<itemsToProcess; i++){
			...
			gatekeeper.authorise(spIndicators[i]).then(function(authorisedReturn){
				...
				console.log('in authorised i = '+ i);
				console.log(authorisedReturn);
				
				gatekeeper.persist(authorisedReturn).then(function(savedArray){
					
					console.log('gatekeeper.persist(authorisedReturn)');
					...
					console.log('uuidSavedIndicator=='+ uuidSavedIndicator);
					dao.get(uuidSavedIndicator).done(function(data){
						if (_WFInstance.indicators.length == 0) {
							_WFInstance.indicators.push(data);
							itemsToProcess--;
							if(itemsToProcess == 0){
								console.log('Before return updatedObjectsArray ==== '+ updatedObjectsArray);
