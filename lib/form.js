...
function create(args) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        var subProcessConf = JSON.xpath("/config/processes/subProcesses[_id eq '" + subProcessId + "']", _WFInstance, {})[0];
        var subprocessType = subProcessConf.type;
                        ...
                        if (!object.model._id.endsWith(':approved') && !object.model._id.endsWith(':rejected')) {
                            ...
                            gatekeeper.persist(docArray).then(function (savedArray) {
                                ...
                                dao.get(mainId).then(function (data) {
                                    ...
                                    gatekeeper.instantiateData(mainId, instantiateSource, indicatorModel, data.model.pending.seq, paramObject, (subProcessConf.periodType.periodic != undefined)).then(function (data) {
        ...
        var loopFunction = function (indicators, counter) {
            ...
            if (indicators.length == 0) {
                ...
                var sdoCreateType = indicators[counter].sequenceCreation;
                if(sdoCreateType == undefined){
                    sdoCreateType = "";
                }
                ...
                if (baseUUID != undefined && baseUUID != '' && baseUUID.length > 0) {
                    ...
                    var existingEntry = JSON.xpath("/subprocesses[_id eq '" + baseUUID + "']/indicators[id eq '" + indicatorId + "']", _WFInstance, {})[0];
                    if(existingEntry == undefined || formCreateType == BLANK_MODEL || sdoCreateType == BLANK_MODEL){
