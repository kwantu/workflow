                    ...
                    var getRecipients = async function(notification) {
                        ...
                        for(var r = 0; r < notification.recipients.length; r++){
                            ...
                            if (notification.recipients[r].role != undefined) {
                                ...
                                var isRepeat = notification.recipients[r]?.email?.field?.code.split(":").length == 5;
                                var emailIds = [];
                                if(isRepeat){
                                    var repeat = model[notification.recipients[r]?.email?.field?.code.split(":")[2]][notification.recipients[r]?.email?.field?.code.split(":")[3]][notification.recipients[r]?.email?.field?.code.split(":")[4]];
                                ...
                                var indObject = JSON.xpath("/indicators[category/term eq '" + indicatorSetId + "']", _WFInstance, {})[0];
                                var model = indObject.model.pending.data[indicatorSetId];
                                var isRepeat = notification.recipients[r]?.sms?.field?.code.split(":").length == 5;
                                var smsNumbers = [];
                                if(isRepeat){
                                    var repeat = model[notification.recipients[r]?.sms?.field?.code.split(":")[2]][notification.recipients[r]?.sms?.field?.code.split(":")[3]][notification.recipients[r]?.sms?.field?.code.split(":")[4]];
                                    for(var rp = 0; rp < repeat.length; rp++){
                                        var rptModel = repeat[rp];
