            ...
            return new Promise(function(resolve, reject) {
                    ...
                    */
                    ...
                    if(notification.sdoKeys != undefined){
                        notification.sdoKeys.forEach(function(k){

                            if(k.field !="" && k.repStr != "" && k.setId !=""){

                                var keyIndicatorUUID = "";
                                subProcessObject.indicators.forEach(function(spInd){
                                    if(spInd.id == k.setId){
                                        keyIndicatorUUID = spInd.instances[0].uuid
                                    }
                                })
                                var indObject = {};
                                _WFInstance.indicators.forEach(function(wfInd){
                                    if(wfInd._id == keyIndicatorUUID){
                                        indObject = wfInd
                                    }
                                })
                                var value = indObject.model.pending.data[k.setId][k.field]
                ...
                helper.getNodeValue(addToRole.userName, _WFInstance, uuid).then(function(userDisplayName) {
                    helper.getNodeValue(addToRole.userId, _WFInstance, uuid).then(function(userId) {
                        helper.getNodeValue(addToRole.roleId, _WFInstance, uuid).then(function(roleId) {
                            ...
                            if(Array.isArray([userDisplayName]) && Array.isArray([userId])){
