    ...
    return {
        ...
        createCommunity: function(_def, uuid, _WFInstance) {
            ...
            return new Promise(function(resolve, reject) {
                ...
                workerObject.senderUserName = _lclx.SESSION.firstName + " " + _lclx.SESSION.lastName;
    ...
    return {
        ...
        sendNotificationWorker: function(notification, _WFInstance, uuid) {
            ...
            return new Promise(function(resolve, reject) {
                    ...
                    var getRecipients = async function(notification) {

                        var recipients = [];
                        if(notification.recipients.length == undefined){
                            notification.recipients = [notification.recipients];
                        }
                        for(var r = 0; r < notification.recipients.length; r++){
                            var item = {};
                            if (notification.recipients[r].role != undefined) {

                                item.role = notification.recipients[r].role;
                                recipients.push(item);

                            } else if (notification.recipients[r].profileRole != undefined) {

                                item.profileRole = {};
                                item.profileRole.role = notification.recipients[r].profileRole.role;
                                item.profileRole.profile = notification.recipients[r].profileRole.profile;
                                recipients.push(item);

                            } else if (notification.recipients[r].subProfileRole != undefined) {

                                item.subProfileRole = {};
                                item.subProfileRole.role = notification.recipients[r].subProfileRole.role;
                                item.subProfileRole.subProfileCategory = notification.recipients[r].subProfileRole.subProfileCategory;
                                item.subProfileRole.profile = notification.recipients[r].subProfileRole.profile;
                                recipients.push(item);

                            } else if (notification.recipients[r].function != undefined) {

                                item.function = {};
                                item.function.users = {};

                                var dataValue = await helper.getNodeValue(notification.recipients[r].function.users, _WFInstance, uuid);

                                item.function.users = dataValue;
                                recipients.push(item);

                                
                            } else if (notification.recipients[r].stepAssignee != undefined) {

                                item.stepAssignee = {};
                                item.stepAssignee.role = notification.recipients[r].stepAssignee.role;
                                recipients.push(item);

                            } else if (notification.recipients[r].stepAssigneeFromStep != undefined) {

                                item.stepAssigneeFromStep = {};
                                var tokens = notification.recipients[r].stepAssigneeFromStep.stepId.split(":");
                                var stepId = tokens[tokens.length - 1];
                                if(stepId == undefined){
                                    continue;
                                }
                                item.stepAssigneeFromStep.stepId = stepId;
                                // calcukate
                                var subProcessObject = JSON.xpath("/subprocesses[_id eq '" + uuid + "']", _WFInstance, {})[0];
                                if(subProcessObject?.step?.id == stepId){
                                    item.stepAssigneeFromStep.userId = subProcessObject?.step.assignedTo.userId;
                                }
                                else{
                                    var historyObject = {};
                                    ...
                                    if(historyObject.assignmentHistory != undefined && historyObject.assignmentHistory.length > 0){
                                        ...
                                        item.stepAssigneeFromStep.userId = obj.userId;
                                    }
                                }
                                if(item.stepAssigneeFromStep.userId != undefined){
                                    recipients.push(item);
                                }
                                

                            } else if (notification.recipients[r].indicator != undefined) {

                                item.indicator = {};
                                
                                var indicatorSetId = notification.recipients[r].indicator.indicatorSetId;
                                var elementId = notification.recipients[r].indicator.elementId;
                                ...
                                item.indicator.userId = retValue;
                                recipients.push(item);

                            }
                            else if (notification.recipients[r]?.email?.setId?.code != undefined && notification.recipients[r]?.email?.field?.code != undefined) {
                                
                                var indicatorSetId = notification.recipients[r].email.setId.code.split(":")[0];
                                var indObject = JSON.xpath("/indicators[category/term eq '" + indicatorSetId + "']", _WFInstance, {})[0];
                                if(indObject == undefined){
                                    continue;
                                }
                                var model = indObject.model.pending.data[indicatorSetId];
                                var isRepeat = notification.recipients[r]?.email?.repeat?.code != undefined && notification.recipients[r]?.email?.repeat?.code.split(":").length == 5;
                                var emailIds = [];
                                if(isRepeat){
                                    var repeat = model[notification.recipients[r]?.email?.repeat?.code.split(":")[2]][notification.recipients[r]?.email?.repeat?.code.split(":")[3]][notification.recipients[r]?.email?.repeat?.code.split(":")[4]];
                                    for(var rp = 0; rp < repeat.length; rp++){
                                        var rptModel = repeat[rp];
                                        var pathToElement = notification.recipients[r]?.email?.field?.code.split(".");
                                    
                                        for (var k = 0; k < pathToElement.length; k++) {
                                            if(rptModel[pathToElement[k]] == undefined){
                                                break;
                                            }
                                            rptModel = rptModel[pathToElement[k]];
                                        }
                                        if(rptModel != undefined && rptModel.length > 0){
                                            emailIds.push(rptModel);
                                        }
                                    }
                                }
                                else{
                                    ...
                                    for (var k = 0; k < pathToElement.length; k++) {
                                        if(model[pathToElement[k]] == undefined){
                                            break;
                                        }
                                        model = model[pathToElement[k]];
                                    }
                            ...
                            else if (notification.recipients[r]?.sms?.setId?.code != undefined && notification.recipients[r]?.sms?.field?.code != undefined) {

                                var indicatorSetId = notification.recipients[r].sms.setId.code.split(":")[0];
                                var indObject = JSON.xpath("/indicators[category/term eq '" + indicatorSetId + "']", _WFInstance, {})[0];
                                if(indObject == undefined){
                                    continue;
                                }
                                var model = indObject.model.pending.data[indicatorSetId];
                                var isRepeat = notification.recipients[r]?.sms?.repeat?.code != undefined && notification.recipients[r]?.sms?.repeat?.code.split(":").length == 5;
                                var smsNumbers = [];
                                if(isRepeat){
                                    var repeat = model[notification.recipients[r]?.sms?.repeat?.code.split(":")[2]][notification.recipients[r]?.sms?.repeat?.code.split(":")[3]][notification.recipients[r]?.sms?.repeat?.code.split(":")[4]];
                                    for(var rp = 0; rp < repeat.length; rp++){
                                        var rptModel = repeat[rp];
                                        var pathToElement = notification.recipients[r]?.sms?.field?.code.split(".");
                                    ...
                                    if(model != undefined && model.length > 0){
                                        smsNumbers.push(model);
                                    }
                                }
                                item.smsNumbers = smsNumbers;
                                recipients.push(item);

                            }
                            else{
                                console.log("Notification - no recipient error ");
                                throw "Notification - no recipient error ";
                            }
                        }
                        ...
                        */
                        ...
                        if (notification.notificationAction != undefined) {
                            ...
                            if (notification.notificationAction.action.URI != undefined) {
                                ...
                                action.notification.notificationAction.action.target = notification.notificationAction.action.target;
                        ...
                        if(typeof notification === "string"){
                            ...
                            dao.getConfig(notification).then(function(notiObject){
                                ...
                                getRecipients(notification).then(function(recipient) {
                                    ...
                                    action.notification.configUUID = notification._id;
    ...
    return {
        ...
        create: function(workerConfig, _WFInstance, uuid) {
            ...
            return new Promise(function(resolve, reject) {
                ...
                action[workerAction].applicationId = app.SCOPE.applicationId;
                action[workerAction].communityId = app.SCOPE.getCommunityId();
                action[workerAction].profileId = app.SCOPE.workflow.profile;
                action[workerAction].subProcessUUID = uuid;
        ...
        addToUserFavourites: function(favBlock, _WFInstance, uuid){
            ...
            return new Promise(function(resolve, reject) {
                ...
                if (userBlock.stepAssigneeFromStep != undefined) {
                    var tokens = userBlock.stepAssigneeFromStep.stepId.split(":");
                    var stepId = tokens[tokens.length - 1];
                    var subProcessObject = JSON.xpath("/subprocesses[_id eq '" + uuid + "']", _WFInstance, {})[0];
                    var historyObject = {};
                    for(var i = 0; i < subProcessObject.history.length; i++){
